            ...
            <div
                ...
                <div class="p-8 border-b border-primary/5">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Account</h2>
                ...
                <form class="p-8 space-y-8" method="post"
                    action="<?= base_url('profile/accounts/update/' . $account['id']) ?>">
                    <?= csrf_field() ?>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <?php
                            $types = [
                                'bank' => ['icon' => 'account_balance', 'label' => 'Bank', 'color' => 'blue'],
                                'e-wallet' => ['icon' => 'account_balance_wallet', 'label' => 'E-wallet', 'color' => 'purple'],
                                'wallet' => ['icon' => 'payments', 'label' => 'Cash', 'color' => 'green']
                            ];
                            foreach ($types as $value => $info):
                                // Tentukan warna latar ikon berdasarkan tipe (bisa dinamis)
                                $bgColor = match ($value) {
                                ...
                                <label class="account-type-card cursor-pointer relative">
                                    ...
                                    <div
                                        class="flex flex-col items-center justify-center p-6 border-2 border-slate-100 dark:border-slate-800 rounded-xl transition-all peer-checked:border-primary peer-checked:bg-primary/10 peer-checked:ring-2 peer-checked:ring-primary/20">
                                        <div
                                            class="w-12 h-12 <?= $bgColor ?> rounded-full flex items-center justify-center mb-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Initial Balance (readonly atau bisa diedit? Biasanya tidak diedit) -->
                        <div class="space-y-2">
                            <label class="text-sm font-semibold text-slate-600 dark:text-slate-400">Balance</label>
                            <div class="relative">
                                <span
                                ...
                                <input type="number" name="balance" value="<?= old('balance', $account['balance']) ?>"
                                    ...
                                    class="w-full bg-slate-100 dark:bg-slate-800 border border-primary/10 rounded-xl focus:ring-primary focus:border-primary text-slate-900 dark:text-white pl-12 pr-4 py-3 transition-all cursor-not-allowed"
                                    placeholder="0,00" />
                            </div>
                            <p class="text-xs text-slate-400 mt-1">Saldo tidak dapat diubah langsung, akan terupdate
                                otomatis melalui transaksi.</p>
                        </div>
