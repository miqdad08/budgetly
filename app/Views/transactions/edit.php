...
<head>
    ...
    <title>Budgetly - Edit Transaction</title>
    <!-- Tailwind & Fonts -->
    ...
    <main class="flex-grow flex items-center justify-center p-4">
        <div
            ...
            <div class="px-8 pt-8 pb-4 flex justify-between items-center">
                <h1 class="text-2xl font-bold text-background-dark dark:text-white">Edit Transaction</h1>
            ...
            <!-- Transaction Form -->
            <form method="post" action="<?= base_url('transactions/update/' . $transaction['id']) ?>" class="p-8">
                <?= csrf_field() ?>
                ...
                <div class="text-center mb-10">
                    ...
                    <div class="relative flex items-center justify-center w-full">
                        <input type="text" name="amount_display" id="amountDisplay"
                            ...
                            placeholder="0,00"
                            value="<?= old('amount') ? number_format(old('amount'), 2, ',', '.') : number_format($transaction['amount'], 2, ',', '.') ?>"
                            inputmode="numeric" />
                    </div>
...
<div class="grid grid-cols-2 gap-4">
    <div>
        <label class="block text-sm font-medium text-background-dark/60 dark:text-white/60 mb-2">Date</label>
        <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
    ...
    <div>
        <label class="block text-sm font-medium text-background-dark/60 dark:text-white/60 mb-2">Account</label>
        <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <?php endif; ?>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                            ...
                            placeholder="What was this transaction for?"><?= old('notes', $transaction['notes']) ?></textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateCategoryOptions(selectedType) {
            ...
            // Set selected category based on old value or transaction data
            const selectedCategory = '<?= old('category_id', $transaction['category_id']) ?>';
            if (selectedCategory) {
                const optionExists = Array.from(categorySelect.options).some(opt => opt.value === selectedCategory);
                if (optionExists) {
        ...
        function adjustFontSize(input, digitCount) {
            let baseSize = 48; // px
            let maxDigits = 10; // mulai mengecil setelah 10 digit
            let minSize = 20; // px
